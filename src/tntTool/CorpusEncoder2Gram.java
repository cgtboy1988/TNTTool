...
public class CorpusEncoder2Gram
{
	...
	public static void main(String[] args)
	{
		...
		for(int x=0; x<fileNames.length; x++)
		{
			...
			try
			{
				...
				String wordSQL = "INSERT IGNORE INTO `corpusDB`.`2Grams` (`corpus`, `number`, `position`, `word`, `pos`, `line`, `vectorWeight`) VALUES (?, ?, ?, ?, ?, ?, ?)";
				String wordSQLValue = ",(?, ?, ?, ?, ?, ?, ?)";
				String wordSQLEnd = ";";
				...
				while((curLine=fileReader.readLine()) != null)
				{
					...
					else
					{
						...
						{
							wordMap.put("word", lastWord + " " + tmp);
							tmp2 = posScanner.next();
							wordMap.put("pos", lastPOS + " " + tmp2);
							lastWord = tmp;
							lastPOS = tmp2;
						}
						else
						{
							wordMap.put("word", lastWord + " ");
							wordMap.put("pos", lastPOS + " " + tmp);
